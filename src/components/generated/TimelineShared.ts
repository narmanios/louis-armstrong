...
export function generateLyricPositions(
  ...
  return lyrics.map((lyric, index) => {
    ...
    const normalizedPosition = (index / Math.max(lyrics.length - 1, 1)) * totalWidth;
...
type CareerTimelineData = {
  ...
  events: Array<{
    ...
    dateText: string;
    event: string;
    categories: Array<'musician' | 'vocalist' | 'bandleader' | 'ambassador' | 'film'>;
    url?: string;
  }>;
};
...
export const roleLines: RoleLine[] = careerTimelineData.roleLines.map((line, idx) => ({
  ...line,
...
export const timelineDots: TLDot[] = careerTimelineData.events.flatMap(event =>
  event.categories.map(category => ({
    ...
    url: event.url,
    outlined: event.id === 82 || event.id === 87 || event.id === 91 || event.id === 93,
  }))
);
...
export const lyricLabels: LyricLabel[] = generateLyricPositions(rawLyrics, spreadFactor);
