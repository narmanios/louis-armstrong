...
const africaTourJsonUrl = new URL('../../../data/africa-tour.json', import.meta.url).href;

export const SectionAfricaTour: React.FC<SectionAfricaTourProps> = ({ className, style }) => {
  const isMobile = useIsMobile();
  const [africaTourImageData, setAfricaTourImageData] = useState<GalleryImage[]>([]);
    ...
    const loadAfricaTourImages = async () => {
      try {
        ...
        const normalized = data.map(item => ({
          id: String(item.id),
          src: item.image_link.startsWith('/') ? item.image_link : `/${item.image_link}`,
  ...
  const selectedImage = africaTourImageData.find(img => img.id === selectedImageId) ?? null;
  const loremText =
          ...
          >
            <div
              style={{
                ...
                display: 'grid',
                gridTemplateColumns: isMobile ? 'repeat(4, 1fr)' : 'repeat(7, 42px)',
                gap: '5px',
                marginBottom: '9px',
              }}
            >
                    ...
                    {africaTourImageData.map(img => (
                      <button
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          position: 'relative',
                          opacity: selectedImageId && selectedImageId !== img.id ? 0.35 : 1,
                          transition: 'opacity 0.25s ease, transform 0.2s ease',
                          transform: selectedImageId === img.id ? 'scale(1.04)' : 'scale(1)',
                          outline:
                            selectedImageId === img.id ? '2px solid rgba(255,255,255,0.9)' : 'none',
                        }}
                      >
                        <img
                          src={img.src}
